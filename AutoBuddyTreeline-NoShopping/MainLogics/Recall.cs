...
namespace AutoBuddy.MainLogics
{
    internal class Recall
    {
        ...
        private readonly Slider minHealth;
        ...
        public Recall(LogicSelector currentLogic, Menu parMenu)
        {
            ...
            minHealth = new Slider("Minimum health % to recall", 20, 0, 100);

            menu.Add("mingold", flatGold);
            menu.Add("goldper", goldPerLevel);
            menu.Add("minHealth", minHealth);
...
AutoBuddy won't recall if you have less gold than needed for next item.
        ...
        private void ShouldRecall()
        {
            ...
            if ((AutoWalker.myHero.Gold > flatGold.CurrentValue + AutoWalker.myHero.Level * goldPerLevel.CurrentValue &&
                AutoWalker.myHero.InventoryItems.Length < 8 &&
                 recallsWithGold <= 30) || AutoWalker.myHero.HealthPercent() < minHealth.CurrentValue)
            {

...
namespace ARAMDetFull.Champions
{
    class Katarina : Champion
    {
        ...
        public override void useR(Obj_AI_Base target)
        {
            if (!R.IsReady())
                return;
            if (R.GetDamage(target) * 8 > target.Health || player.CountEnemiesInRange(450) > 1 || player.HealthPercent < 25)
                R.Cast();
        }
        ...
        public override void killSteal()
        {
            ...
            if (Q.IsReady() && /*target.isKillable(target, 1)*/ Q.IsKillable(target) &&
            ...
            if (Q.IsReady() && E.IsReady() && Q.IsKillable(target) && W.IsKillable(target)
            ...
            if (Q.IsReady() && E.IsReady() && W.IsReady() && W.IsKillable(target)/*
                ObjectManager.Player.IsKillable(target,
        ...
        {
            //if (!ObjectManager.Player.CanMove()) return;
            var allMinions = EntityManager.MinionsAndMonsters.GetLaneMinions(EntityManager.UnitTeam.Enemy, ObjectManager.Player.ServerPosition, Q.Range);
            var useQ = true;
            var useW = true;

            if (useQ && Q.IsReady())
            {
                foreach (var minion in allMinions.Where(minion => minion.IsValidTarget() && Prediction.Health.GetPrediction(minion, (int)(ObjectManager.Player.Distance(minion) * 1000 / 1400))
                < 0.75 * ObjectManager.Player.GetSpellDamage(minion, SpellSlot.Q)))
                {
                    Q.Cast(minion);
                    return;
                }
        ...
        private float GetComboDamage(Obj_AI_Base enemy)
        {
            ...
            if (R.IsReady())
                damage += ObjectManager.Player.GetSpellDamage(enemy, SpellSlot.R) * 8;
            return (float)damage;
        }
    }
}
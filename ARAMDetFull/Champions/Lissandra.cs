...
namespace ARAMDetFull.Champions
{
    class Lissandra : Champion
    {
        ...
        public override void setUpSpells()
        {
            ...
            E = new Spell.Skillshot(SpellSlot.W, 1050, SkillShotType.Linear, 250, 850, 125);
        ...
        {
            Dictionary<SharpDX.Vector3, int> maxhit = (from hero in EntityManager.Heroes.Enemies.Where(h => h.IsValidTarget() && !h.IsInvulnerable && Vector3.Distance(h.ServerPosition, player.ServerPosition) < Q.Range)
                                                       select Q.GetPrediction(hero) into prediction
                                                       where prediction.CollisionObjects.Length > 0 && prediction.HitChance >= HitChance.High
                                                       let enemieshit = prediction.CollisionObjects.Where(x => x is AIHeroClient)
                                                       select prediction).ToDictionary(prediction => prediction.CastPosition, prediction => prediction.CollisionObjects.Length);
            
            var bestpair = maxhit.OrderByDescending(x => x.Value).FirstOrDefault();
            if (bestpair.Value > 0)
            {
            ...
            if (distbw > Q.Range && distbw < Q.Range)
            {
                ...
                if ((testQ.HitChance == HitChance.Collision || collobjs.Length > 0) && collobjs.All(x => x.IsTargetable))
                {

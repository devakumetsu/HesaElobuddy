...
namespace ARAMDetFull.Champions
{
    class Lucian : Champion
    {
        ...
        public Prediction.Position.PredictionData Linear { get; private set; }
        ...
        private void AfterAttack(AttackableUnit unit, EventArgs args/*, AttackableUnit target*/)
        {
            ...
            if (Q.IsReady())
            {
                useQonTarg((AIHeroClient)unit, QhitChance.medium);
            }
        ...
        public QhitChance hitChOnTarg(AIHeroClient target, Obj_AI_Base onTarg)
        {
            ...
            //var predTarPos = Prediction.Position.GetPrediction(target, 035).UnitPosition.To2D();
            var predTarPos = Prediction.Position.GetPrediction(target, Linear).UnitPosition.To2D();
            //Q.GetPrediction(enemyGhost).HitChance >= HitChance.High)
        ...
        public Polygon getPolygonOn(Obj_AI_Base target, float bonusW = 0)
        {
            ...
            Vector2 rTpos = Prediction.Position.GetPrediction(target, Linear).UnitPosition.To2D();

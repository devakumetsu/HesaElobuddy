    ...
    class Polygon
    {
        ...
        public void Draw(System.Drawing.Color color, int width = 1)
        {
            for (var i = 0; i <= Points.Count - 1; i++)
            {
                ...
                Vector2 from2D;
                var from = Drawing.WorldToScreen(Points[i].To3D(), out from2D);
                Vector2 to2D;
                var to = Drawing.WorldToScreen(Points[nextIndex].To3D(), out to2D);
                Drawing.DrawLine(from2D[0], from2D[1], to2D[0], to2D[1], width, color);
            }
        }
